...
trait BootstrapChecks {
  object logger extends Logger {
    override protected def _logger = LoggerFactory.getLogger("bootactions")
  }
...
class SequentialImmediateBootStrapChecks(checkActions:BootstrapChecks*) extends BootstrapChecks {
  ...
  override def checks() : Unit = checkActions.zipWithIndex.foreach { case (check,i) =>
    ...
    if(logger.isDebugEnabled) {
      logger.info(s"[#${i}] ${check.description}")
    } else {
      logger.info(s"${check.description}")
    }
    check.checks
    logger.debug(s"[#${i}] Check OK in [${formater.print(new Duration(System.currentTimeMillis - start).toPeriod)}]")
  }
