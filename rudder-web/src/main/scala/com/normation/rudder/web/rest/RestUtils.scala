...
object ApiVersion {
  ...
  def fromRequest(req:Req, availableVersions : List[Int]) : Box[ApiVersion] = {
    req.header("X-API-VERSION") match {
      case Full(value) => fromString(value, availableVersions)
      case eb: EmptyBox => eb ?~ ("Error when getting header X-API-VERSION")
    }
  }

  def fromString (version : String, availableVersions : List[Int]) : Box[ApiVersion] = {
