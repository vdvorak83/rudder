...
class ReportsJdbcRepository(jdbcTemplate : JdbcTemplate) extends ReportsRepository {
  ...
  def findReportsByNode(nodeId : NodeId, ruleId : Option[RuleId], serial : Option[Int], beginDate: Option[DateTime], endDate: Option[DateTime]): Seq[Reports] = {
    var query = baseQuery + " and nodeId = ? "
    var array = mutable.Buffer[AnyRef](nodeId.value)
    
    ruleId match {
      case None => 
      case Some(rule) => query = query + " and ruleId = ?"; array += rule.value
