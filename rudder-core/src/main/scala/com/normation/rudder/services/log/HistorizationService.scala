...
class HistorizationServiceImpl(
  ...
  def updatePINames() : Box[Unit] = {
    ...
    // againt, it should not fail (but report an error nonetheless)
      ...
      val changed = directives.filter { case (directive, userPT, policyPackage) => 
          registered.get(directive.id.value) match {
              ...
              case Some(entry) => 
                  ...
                  entry.techniqueHumanName != policyPackage.name ||
                  entry.policyPackageName != userPT.techniqueName.value ||
                  entry.policyPackageDescription != policyPackage.description ||
                  entry.policyPackageVersion != directive.techniqueVersion.toString )
